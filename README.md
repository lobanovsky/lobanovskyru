...
Информационный сайт на Astro. Сборка и тесты выполняются в Docker; готовую статику обслуживает Caddy. На сервере HTTPS и маршрутизацией управляет существующий Traefik. `basket.lobanovsky.ru` обслуживается тем же контейнером: Caddy отдаёт страницу корзины по её поддомену. Страница — `src/pages/basket.astro`, фотографии — `public/basket/media/`. Для локального просмотра откройте `/basket/`.
